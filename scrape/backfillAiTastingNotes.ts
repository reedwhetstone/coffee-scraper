...
class AiTastingNotesBackfill {
  ...
  async fetchRecordsNeedingAiTastingNotes(): Promise<CoffeeRecord[]> {
    this.log('Fetching records where stocked=true and ai_tasting_notes is null...');
    
    const { data, error } = await supabase
      .from('coffee_catalog')
      .select('id, name, description_long, description_short, cupping_notes, source')
      .eq('stocked', true)
      .is('ai_tasting_notes', null)
      .order('id');
    ...
    this.log(`Found ${data?.length || 0} records needing AI tasting notes`);
    return data || [];
  }
  ...
  async processRecord(record: CoffeeRecord): Promise<boolean> {
    ...
    try {
      ...
      // Check if we have tasting data to work with
      if (!record.description_long && !record.description_short && !record.cupping_notes) {
        this.log(`  Skipping - no tasting data available`);
        this.errorCount++;
        return false;
      }
      ...
      const response = await this.geminiClient.generateAiTastingNotes(
        ...
        record.cupping_notes
      );

      if (!response.success) {
        this.log(`  Error generating AI tasting notes: ${response.error}`);
        this.errorCount++;
        return false;
      }

      const aiTastingNotes = response.data;

      // Update the database
      const { error: updateError } = await supabase
        .from('coffee_catalog')
        .update({ 
          ai_tasting_notes: aiTastingNotes,
      ...
      this.log(`  Success - Generated tasting notes: ${JSON.stringify(aiTastingNotes).substring(0, 100)}...`);
      this.successCount++;
      return true;
  ...
  async run(): Promise<void> {
    try {
      this.log('Starting AI tasting notes backfill process...');
      
      const records = await this.fetchRecordsNeedingAiTastingNotes();
      
      if (records.length === 0) {
        this.log('No records need AI tasting notes. Exiting.');
        return;
      }
      ...
      this.log('\n=== AI TASTING NOTES BACKFILL COMPLETE ===');
...
if (isMainModule) {
  const backfill = new AiTastingNotesBackfill();
  
  backfill.run()
    .then(() => {
      console.log('AI tasting notes backfill completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      console.error('AI tasting notes backfill failed:', error);
      process.exit(1);
    });
}

export default AiTastingNotesBackfill;